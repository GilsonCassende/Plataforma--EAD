...
class AuthController
{
    ...
    public function registrar($nome, $email, $senha, $confirmSenha, $role = 'aluno'): array
    {
        ...
        $resultado = $this->userModel->criar($nome, $email, $senhaHash, $role);
        if (!$resultado['sucesso']) {
            return $resultado;
        }

        if (function_exists('registrar_log')) {
            registrar_log('signup_created', 'Conta criada e ativada automaticamente para ' . $email);
        }

        return [
            'sucesso' => true,
            'mensagem' => 'Cadastro realizado com sucesso. Já pode fazer login.',
        ];
    }
